...
def process_pdf(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        # Return the extracted text, encoded in UTF-8
        return func.HttpResponse(extracted_text.encode('utf-8'), status_code=200)
    except Exception as e:
        logging.error(f"Error: {str(e)}")
        return func.HttpResponse(f"Error occurred: {str(e)}", status_code=500, charset='utf-8')
...
def extract_text_from_pdf(pdf_bytes):
    ...
    for page_num in range(len(doc)):
        ...
        for img_index, img in enumerate(page.get_images()):
            ...
            # Open the image with PIL and apply OCR
            image = Image.open(io.BytesIO(image_bytes))
            image_text = reader.readtext(image)
            # Concatenate the OCR results
            full_text += ' '.join([item[1] for item in image_text]) + "\n"
